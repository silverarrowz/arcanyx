...
def get_models_to_try() -> List[str]:
    ...
    fallback_model = os.getenv("GEMINI_FALLBACK_MODEL", "gemini-3-flash-preview").strip()

    models: List[str] = []

    for model in [primary_model, fallback_model, "gemini-3-flash-preview"]:
...
def build_gemini_request_body(payload: DreamInterpretRequest) -> dict:
    ...
    system_prompt = (
        ...
        "Интерпретация должна звучать как глубокое, но понятное психологическое размышление. "
        "Обращай внимание на символы, внутренние противоречия, желания, страхи, границы, переходы, отношения с собой и миром. "
        "Не используй диагнозы, мистическое запугивание и категоричные утверждения. "
        "Формулируй мягко: 'может указывать', 'похоже', 'возможно', 'сон может отражать'. "
        "title: краткий заголовок. "
        "interpretation: 4-5 предложений. "
        "symbols: массив из 3-4 простых конкретных символов из самого сна на русском. "
        "Это должны быть ключевые объекты, места, существа, действия или события из сна "
